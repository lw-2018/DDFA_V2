...
def method_search2(from_values, arr, to_values1, to_values2, h, w):
    ...
    return datax.reshape(h,w), datay.reshape(h,w), datam.reshape(h,w).astype(bool)
...
def get_7_flow(imgs,savepath,ver_lsts):
    parser = argparse.ArgumentParser(description='The demo of still image of 3DDFA_V2')
    parser.add_argument('-c', '--config', type=str, default='configs/mb1_120x120.yml')
    parser.add_argument('-i', '--img_fp', type=str, default='/home/wei/gy/code/3DDFA_V2/examples/results/72.png_448_ori.jpg')
    parser.add_argument('-r', '--img_ref', type=str, default='/home/wei/gy/code/3DDFA_V2/examples/results/268.png_448_ori.jpg')
    ...
    args = parser.parse_args()
    args.img_fp = savepath+'4.png'
    img_ref_tri = main(args,imgs[3],ver_lsts[3])
    flow_7 = []
    mask_7 = []
    count_pic = 0
    for img in imgs:
        args.img_fp = savepath+str(count_pic+1)+'.png'
        img_tri = main(args,img,ver_lsts[count_pic])
        count_pic+=1
        flow, mask = optical_cal(img_tri, img_ref_tri)
        flow_7.append(flow)
        mask_7.append(mask)
    return flow_7,mask_7

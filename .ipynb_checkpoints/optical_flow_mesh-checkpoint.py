...
import numpy as np
import yaml
sys.path.append('/home/wei/exp/data_process/DDFA_V2')
...
def get_7_flow(imgs,savepath):
    ...
    args = parser.parse_args()
    args.img_fp = savepath+'3.png'
    img_ref_tri = main(args,imgs[3])
    flow_7 = []
    count_pic = 0
    for img in imgs:
        args.img_fp = savepath+str(count_pic)+'.png'
        img_tri = main(args,img)
        count_pic+=1
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='The demo of still image of 3DDFA_V2')
    parser.add_argument('-c', '--config', type=str, default='configs/mb1_120x120.yml')
    parser.add_argument('-i', '--img_fp', type=str, default='/data2/wei/vox-312/LR/id02240/S2npOzggL3o/im2.png')
    parser.add_argument('-r', '--img_ref', type=str, default='/data2/wei/vox-312/LR/id02240/S2npOzggL3o/im4.png')
    parser.add_argument('-m', '--mode', type=str, default='gpu', help='gpu or cpu mode')
    parser.add_argument('-o', '--opt', type=str, default='3d',
                        choices=['2d_sparse', '2d_dense', '3d', 'depth', 'pncc', 'uv_tex', 'pose', 'ply', 'obj'])
    parser.add_argument('--show_flag', type=str2bool, default='true', help='whether to show the visualization result')
    ...
    img_ref_tri = main_s(args)
  #  flow2 = optical_cal(img_tri, img_ref_tri)
   #print(flow2.shape)
   # print(img.shape)
    flow = np.load('/data2/wei/vox/GT/id02240/S2npOzggL3o/flow.npy')
    flow = flow[2].astype(int)
   # viz_flow(flow.astype(float))
    outpath2 = f'examples/results/_flow_ref.jpg'
    color2 = viz_flow(flow.astype(float))
    cv2.imwrite(outpath2, color2)
    print(flow.shape)
